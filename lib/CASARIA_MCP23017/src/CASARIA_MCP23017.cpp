...
void CASARIA_MCP23017::setAddress(int a0, int a1, int a2){
...
void CASARIA_MCP23017::setOutput(int num){
    if(num > 8){
        outputMap[1] ^= (1<<(num-1));
    }else{
        outputMap[0] ^= (1<<(num-1));
...
void CASARIA_MCP23017::turnOffRelay(int relay){
    relayOp(relay, 1);
}
...
int CASARIA_MCP23017::readRelayStatus(int relay){
    if(relay < 8){
        int rbit = (1<<(relay-9));
...
int CASARIA_MCP23017::readInputStatus(int input){
    if(!iosetCustom){
        input += relayCount;
    }
...
int CASARIA_MCP23017::readAllInputs(){
